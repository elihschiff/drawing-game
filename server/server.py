...
def getwords():
    ...
    msg = {"type": "getwords", "drawer": False, "words": random.sample(words, 5)}
...
class SimpleChat(WebSocket):
    def handleMessage(self):
        ...
        if (
            data["type"] == "msg"
            and data["msgtype"] == "guess"
            and data["msg"] == game["word"]
        ):
            self.sendMessage(json.dumps({"type": "system", "msg": "correct"}))
            for client in clients:
                if client != self:
                    client.sendMessage(
                        json.dumps(
    ...
    def handleConnected(self):
        ...
        if game["drawer"] == None:
            ...
            self.sendMessage(
                json.dumps(
                    {"type": "getwords", "drawer": False, "words": [game["word"]]}
                )
            )
